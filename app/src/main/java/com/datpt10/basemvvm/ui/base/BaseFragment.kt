...
import com.datpt10.basemvvm.BR
import com.datpt10.basemvvm.R
import com.datpt10.basemvvm.utils.DialogUtils
        ...
        viewModel.apply {
            errorMessage.observe(viewLifecycleOwner, Observer {
                if (it != null && it.isNotBlank()) {
                    handleShowMessage(it)
                }
            })
        }
    }

    fun logEvent(key: String, data: Bundle? = null) {
        firebaseAnalytics.logEvent(key,data)
    }


    open fun handleShowMessage(message: String) {
        alertDialog = DialogUtils.showMessage(
            context,
            message = message,
            textPositive = getString(R.string.message_ok)
        )
    }
